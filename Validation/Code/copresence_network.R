...
144-sum(ties==t(ties))  # there are 14 unreciprocated ties

sum(ties==1)  # 47 positive ties
sum(ties==-1) # 48 negative ties

sum(ties+t(ties)==2)   # 46 reciprocated positive ties
sum(ties+t(ties)==-2)  # 42 reciprocated negative ties

sum(ties+t(ties)==-1)  # 10/2 = 12 of the unreciprocated ties are n.s. and negative
sum(ties+t(ties)==1)   # 2/2 = 2 of the unreciprocated ties are n.s. and positive
...
ss[result[1]$`1`]  # "BH" "DH" "OC" "WE"
ss[result[2]$`2`]  # "WB"
ss[result[3]$`3`]  # "BY" "RG" "SC"
ss[result[4]$`4`]  # "CF" "ME" "RB" "YC"
